defmodule SymphonyElixir.Linear.Client do
    ...
    cond do
      ...
      is_binary(tracker.project_slug) ->
        with {:ok, assignee_filter} <- routing_assignee_filter() do
          do_fetch_by_project(tracker.project_slug, tracker.active_states, assignee_filter)
        end
  ...
  def fetch_issues_by_states(state_names) when is_list(state_names) do
    ...
    if normalized_states == [] do
      ...
      cond do
        ...
        is_binary(tracker.project_slug) ->
          do_fetch_by_project(tracker.project_slug, normalized_states, nil)
  ...
  defp do_fetch_by_project(project_slug, state_names, assignee_filter) do
    do_fetch_by_project_page(project_slug, state_names, assignee_filter, nil, [])
  end

  defp do_fetch_by_project_page(project_slug, state_names, assignee_filter, after_cursor, acc_issues) do
    with {:ok, body} <-
           graphql(@query, %{
      ...
      case next_page_cursor(page_info) do
        {:ok, next_cursor} ->
          do_fetch_by_project_page(project_slug, state_names, assignee_filter, next_cursor, updated_acc)
