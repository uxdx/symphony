defmodule SymphonyElixir.CLITest do
  ...
  test "returns the guardrails acknowledgement banner when the flag is missing" do
    ...
    deps = %{
      ...
      preflight_workflow_file: fn _path ->
  ...
  test "defaults to WORKFLOW.md when workflow path is missing" do
    deps = %{
      ...
      preflight_workflow_file: fn _path -> :ok end,
  ...
  test "returns preflight error before setting workflow or starting app" do
    parent = self()
    ...
    deps = %{
      ...
      set_logs_root: fn _path -> :ok end,
      set_server_port_override: fn _port -> :ok end,
      preflight_workflow_file: fn _path -> {:error, "[todo-code] tracker.mystery: unknown field"} end,
      ensure_all_started: fn ->
        send(parent, :started)
        {:ok, [:symphony_elixir]}
      end
    ...
    assert {:error, message} = CLI.evaluate([@ack_flag, "WORKFLOW.md"], deps)
    assert message == "Workflow preflight failed: [todo-code] tracker.mystery: unknown field"
    refute_received :workflow_set
    refute_received :started
  end
end
